          ...
          >
            <div className="flex-1">
              <Field label="Template Name">
                <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Mobile App Proposal" autoFocus />
              </Field>
            </div>
          ...
          <Card key={t.id} className="p-[var(--spacing-lg)] flex flex-col">
            <div className="flex items-start justify-between gap-[var(--spacing-sm)]">
              <div className="min-w-0">
                ...
                {t.isDefault && <span className="text-[var(--text-xs)] text-[var(--color-primary)]">Default template</span>}
              </div>
            </div>
            {t.description && <p className="text-[var(--text-sm)] text-[var(--color-text-muted)] mt-[var(--spacing-xs)] flex-1">{t.description}</p>}
            <p className="text-[var(--text-xs)] text-[var(--color-text-subtle)] mt-[var(--spacing-sm)]">{t.sections.length} sections</p>
