...
function AddProjectForm({ defaultClientId, onDone }: { defaultClientId?: string; onDone: () => void }) {
  ...
  const {
    ...
    defaultValues: { clientId: defaultClientId, status: 'ACTIVE' },
  })
              ...
              <li key={project.id}>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-[var(--text-xs)] text-[var(--color-text-muted)]">{clientLabel(project.clientId)}</p>
                  </div>
                  <span className="text-[var(--text-xs)] text-[var(--color-text-muted)] shrink-0">{project.status}</span>
                </Link>
              </li>
            ))}
          </ul>
