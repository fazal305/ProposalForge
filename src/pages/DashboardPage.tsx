...
const PIPELINE_STATUSES: ProposalStatus[] = ['DRAFT', 'SENT', 'VIEWED', 'APPROVED', 'REJECTED']
...
export function DashboardPage() {
  ...
  const projectName = (projectId: string) => projects.find((p) => p.id === projectId)?.name ?? 'Untitled project'
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-[var(--spacing-sm)] mb-[var(--spacing-xl)]">
        {PIPELINE_STATUSES.map((status) => (
          <Card key={status} className="p-[var(--spacing-md)]">
            ...
            <p className="text-[var(--text-2xl)] font-semibold mt-[var(--spacing-xs)] text-[var(--color-text)]">{counts[status] ?? 0}</p>
          </Card>
        ))}
              ...
              <li key={p.id}>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-[var(--text-xs)] text-[var(--color-text-muted)] truncate">{clientName(p.projectId)}</p>
                  </div>
                  <div className="flex items-center gap-[var(--spacing-md)] shrink-0">
                    <span className="text-[var(--text-sm)] text-[var(--color-text)]">{formatCurrency(p.total, p.currency)}</span>
