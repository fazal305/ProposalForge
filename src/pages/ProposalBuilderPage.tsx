...
const TABS = ['Sections', 'Scope', 'Pricing', 'Timeline', 'Payment Schedule'] as const
        ...
        <nav className="flex gap-[var(--spacing-sm)]" role="tablist" aria-label="Proposal builder sections">
          {TABS.map((t) => (
            <button
              ...
              aria-selected={tab === t}
              onClick={() => setTab(t)}
              className={`px-[var(--spacing-sm)] py-[var(--spacing-sm)] text-[var(--text-sm)] font-medium border-b-2 transition-colors duration-[var(--duration-fast)] whitespace-nowrap ${
                tab === t
