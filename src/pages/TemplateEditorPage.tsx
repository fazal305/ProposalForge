...
function SectionEditor({ templateId, section }: { templateId: string; section: TemplateSection }) {
  ...
  const insertVariable = (token: string) => {
    ...
    updateSection(templateId, section.id, { contentMd: next })
    ...
    <Card className={`p-[var(--spacing-lg)] ${section.enabled ? '' : 'opacity-60'}`}>
      <div className="flex flex-wrap items-center justify-between gap-[var(--spacing-sm)] mb-[var(--spacing-sm)]">
        <div className="flex items-center gap-[var(--spacing-sm)] flex-1 min-w-[200px]">
          ...
          <select
            value={section.type}
            onChange={(e) => updateSection(templateId, section.id, { type: e.target.value as TemplateSection['type'] })}
        ...
        <div className="flex items-center gap-[var(--spacing-2xs)]">
          ...
          <Button
            size="sm"
            variant="ghost"
            onClick={() => updateSection(templateId, section.id, { enabled: !section.enabled })}
          >
      ...
      {previewing ? (
        ...
        <>
          ...
          <Textarea
            ...
            onChange={(e) => updateSection(templateId, section.id, { contentMd: e.target.value })}
            rows={5}
    ...
    <div>
      <PageHeader
        ...
        actions={
          <Button
            variant="primary"
            onClick={() =>
              addSection(template.id, { key: `custom_${Date.now()}`, title: 'New Section', type: 'STATIC', contentMd: '', enabled: true })
            }
          >
            Add Section
          </Button>
      ...
      <Card className="p-[var(--spacing-lg)] mb-[var(--spacing-lg)] grid sm:grid-cols-2 gap-[var(--spacing-md)]">
        <Field label="Template Name">
          <Input value={template.name} onChange={(e) => updateTemplateMeta(template.id, { name: e.target.value })} />
        </Field>
        <Field label="Description">
          <Input
            value={template.description ?? ''}
            onChange={(e) => updateTemplateMeta(template.id, { description: e.target.value })}
          />
        </Field>
      </Card>
