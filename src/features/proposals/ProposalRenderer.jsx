...
function groupScopeItems(items) {
  const map = new Map()
...
function SectionBody({ sectionKey, contentMd, proposal, context }) {
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-current/10">
                ...
                <td className="py-1.5 text-right font-medium w-32">
            ...
            {[...proposal.paymentSchedules]
              ...
              .map((entry) => (
                <tr key={entry.id} className="border-b border-current/10">
                  ...
                  <td className="py-1.5 text-right font-medium w-32">
                    {formatCurrency(entry.amount, proposal.currency)}
                  </td>
                </tr>
              ))}
          </tbody>
...
export function ProposalRenderer({ proposal, context, theme, developerLogoDataUrl }) {
    ...
    >
      ...
      >
        {developerLogoDataUrl && (
          <img
            src={developerLogoDataUrl}
            alt=""
            className="h-12 mb-4"
            style={{
              margin: theme.headerStyle === 'centered' ? '0 auto 16px' : '0 0 16px',
            }}
          />
        )}
      ...
      <div
        style={{
          padding: spacingMap[theme.spacing],
        }}
      ...
      >
        {sections
          .filter(
            (s) =>
              s.contentMd.trim().length > 0 ||
              ['scope_of_work', 'timeline_milestones', 'investment_pricing', 'payment_schedule'].includes(s.sectionKey),
          )
          .map((section) => (
            <section key={section.id}>
              <h2
                ...
                className="text-[20px] font-semibold mb-3 pb-2 border-b"
              >
                {section.title}
              </h2>
