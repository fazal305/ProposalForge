...
function groupBy(items: ScopeItem[]): [string, ScopeItem[]][] {
...
export function ScopeBuilder({ proposalId, scopeItems }: { proposalId: string; scopeItems: ScopeItem[] }) {
  ...
  return (
    <div className="space-y-[var(--spacing-lg)]">
      <Card className="p-[var(--spacing-lg)]">
        <h3 className="text-[var(--text-sm)] font-semibold text-[var(--color-text)] mb-[var(--spacing-sm)]">Add Scope Item</h3>
        <form
          onSubmit={(e) => {
            e.preventDefault()
      ...
      {groups.length === 0 ? (
        <EmptyState title="No scope items yet" description="Add grouped scope items above to build out what's included." />
      ) : (
        groups.map(([group, items]) => (
          <Card key={group} className="p-[var(--spacing-lg)]">
            <h3 className="text-[var(--text-base)] font-semibold text-[var(--color-text)] mb-[var(--spacing-sm)]">{group}</h3>
            <ul className="space-y-[var(--spacing-sm)]">
              {items.map((item) => (
                <li key={item.id} className="flex items-start gap-[var(--spacing-sm)] border-t border-[var(--color-border)] pt-[var(--spacing-sm)] first:border-t-0 first:pt-0">
                  <div className="flex flex-col gap-[var(--spacing-3xs)] pt-1">
                    <button type="button" aria-label="Move up" onClick={() => move(item.id, -1)} className="text-[var(--color-text-muted)] hover:text-[var(--color-text)] text-xs">↑</button>
                    <button type="button" aria-label="Move down" onClick={() => move(item.id, 1)} className="text-[var(--color-text-muted)] hover:text-[var(--color-text)] text-xs">↓</button>
                  </div>
                  <input
                    type="checkbox"
                    checked={item.included}
                    onChange={(e) => updateScopeItem(proposalId, item.id, { included: e.target.checked })}
                  ...
                  <div className="flex-1 min-w-0">
                    <Input
                      value={item.title}
                      onChange={(e) => updateScopeItem(proposalId, item.id, { title: e.target.value })}
                      className={item.included ? '' : 'line-through text-[var(--color-text-subtle)]'}
                    />
                    <Textarea
                      value={item.description ?? ''}
                      onChange={(e) => updateScopeItem(proposalId, item.id, { description: e.target.value })}
                  ...
                  <div className="flex items-center gap-[var(--spacing-2xs)] shrink-0">
                    <Input
                      type="number"
                      value={item.quantity}
                      onChange={(e) => updateScopeItem(proposalId, item.id, { quantity: Number(e.target.value) })}
                      className="w-16"
                      aria-label="Quantity"
                    />
                    <Input
                      value={item.unit}
                      onChange={(e) => updateScopeItem(proposalId, item.id, { unit: e.target.value })}
                      className="w-20"
                      aria-label="Unit"
                    />
                  </div>
