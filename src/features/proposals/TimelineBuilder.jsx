...
export function TimelineBuilder({ proposalId, milestones }) {
              ...
              <Card className="p-[var(--spacing-lg)] flex gap-[var(--spacing-sm)]">
                <div className="flex flex-col gap-[var(--spacing-3xs)] pt-1">
                  <button
                    ...
                    disabled={index === 0}
                    onClick={() => move(m.id, -1)}
                    className="text-[var(--color-text-muted)] disabled:opacity-30 hover:text-[var(--color-text)] text-xs"
                  >
                    ↑
                  </button>
                  <button
                ...
                <div className="flex-1 grid sm:grid-cols-2 gap-[var(--spacing-sm)]">
                  <Field label="Name">
                    <Input
                      value={m.name}
                      onChange={(e) =>
                        updateMilestone(proposalId, m.id, {
                          name: e.target.value,
                        })
                      }
                    />
