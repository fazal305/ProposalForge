...
export function Field({
  ...
  return (
    <label htmlFor={htmlFor} className="block">
      <span className="block text-[var(--text-sm)] font-medium text-[var(--color-text)] mb-[var(--spacing-2xs)]">{label}</span>
      {children}
      {hint && !error && <span className="block mt-[var(--spacing-3xs)] text-[var(--text-xs)] text-[var(--color-text-subtle)]">{hint}</span>}
      {error && (
