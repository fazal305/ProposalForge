...
export function calcSubtotal(items: PricingLineItem[]): number {
...
export function calcTaxAmount(subtotal: number, discountAmount: number, tax: TaxInput | null): number {
...
export interface PaymentScheduleEntryInput {
  label: string
  percent: number
}

export interface PaymentScheduleEntry extends PaymentScheduleEntryInput {
  amount: number
}
 ...
 */
export function calcPaymentSchedule(total: number, entries: PaymentScheduleEntryInput[]): PaymentScheduleEntry[] {
  if (entries.length === 0) return []
  const withAmounts = entries.map((entry) => ({ ...entry, amount: round2(total * (entry.percent / 100)) }))
  const allocated = round2(withAmounts.slice(0, -1).reduce((sum, e) => sum + e.amount, 0))
  const last = withAmounts[withAmounts.length - 1]
  withAmounts[withAmounts.length - 1] = { ...last, amount: round2(total - allocated) }
  return withAmounts
}

export function paymentSchedulePercentTotal(entries: PaymentScheduleEntryInput[]): number {
