...
import { calcDiscountAmount, calcPaymentSchedule, calcSubtotal, calcTaxAmount, calcTotals, paymentSchedulePercentTotal } from './pricing'

describe('calcSubtotal', () => {
  it('sums quantity × unit price across line items', () => {
    expect(calcSubtotal([{ quantity: 1, unitPrice: 150000 }, { quantity: 6, unitPrice: 10000 }])).toBe(210000)
  })
  it('returns 0 for no items', () => {
    expect(calcSubtotal([])).toBe(0)
  })
  it('avoids floating point drift', () => {
    expect(calcSubtotal([{ quantity: 3, unitPrice: 0.1 }])).toBe(0.3)
  })
})

describe('calcDiscountAmount', () => {
  it('applies a flat discount', () => {
    expect(calcDiscountAmount(1000, { type: 'FLAT', value: 150 })).toBe(150)
  })
  it('applies a percent discount', () => {
    expect(calcDiscountAmount(1000, { type: 'PERCENT', value: 10 })).toBe(100)
  })
  it('never discounts more than the subtotal', () => {
    expect(calcDiscountAmount(100, { type: 'FLAT', value: 500 })).toBe(100)
  })
  it('returns 0 when discount is null or non-positive', () => {
    expect(calcDiscountAmount(1000, null)).toBe(0)
    expect(calcDiscountAmount(1000, { type: 'FLAT', value: 0 })).toBe(0)
  })
})
...
describe('calcTaxAmount', () => {
  it('taxes the post-discount amount, not the subtotal', () => {
    ...
    expect(calcTaxAmount(1000, 200, { ratePercent: 10 })).toBe(80)
  })
...
describe('calcTotals', () => {
  it('matches the worked example from the brief', () => {
    ...
    const totals = calcTotals(
      [{ quantity: 1, unitPrice: 150000 }, { quantity: 1, unitPrice: 25000 }, { quantity: 6, unitPrice: 10000 }],
      { type: 'FLAT', value: 15000 },
      { ratePercent: 5 },
    )
...
describe('paymentSchedulePercentTotal', () => {
  it('sums entry percentages', () => {
    expect(paymentSchedulePercentTotal([{ label: 'A', percent: 50 }, { label: 'B', percent: 50 }])).toBe(100)
  })
})
