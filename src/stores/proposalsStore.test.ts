  ...
  return useProposalsStore.getState().createProposal(
    {
      ...
      currency: 'USD',
      pricingMode: 'FIXED',
      sections: [{ sectionKey: 'cover', title: 'Cover', contentMd: 'Hello {{client.name}}', orderIndex: 0 }],
    },
    'Q-1000',
  )
}
...
describe('pricing recalculation', () => {
  it('recomputes totals whenever a pricing item changes', () => {
    const p = makeProposal()
    useProposalsStore.getState().addPricingItem(p.id, { name: 'Website', description: null, quantity: 2, unitPrice: 100 })
...
describe('duplicateProposal', () => {
  it('creates an independent draft copy without mutating the original', () => {
    ...
    useProposalsStore.getState().addPricingItem(original.id, { name: 'Website', description: null, quantity: 1, unitPrice: 500 })
    ...
    useProposalsStore.getState().updatePricingItem(copy.id, copy.pricingItems[0].id, { unitPrice: 999 })
...
describe('sendProposal / status transitions', () => {
  ...
  it('records a client approval and updates status', () => {
    ...
    expect(approved.approvals[0]).toMatchObject({ decision: 'APPROVED', clientName: 'Jordan Blake', comment: 'Looks great' })
  })
})
