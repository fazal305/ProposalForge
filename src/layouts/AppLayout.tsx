    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[var(--color-background)]">
      ...
      >
        <div className="px-[var(--spacing-lg)] py-[var(--spacing-lg)]">
          ...
          <p className="text-[var(--text-xs)] text-[var(--color-text-subtle)] mt-[var(--spacing-3xs)]">Built once. Used every time.</p>
        </div>
        <nav className="flex-1 px-[var(--spacing-sm)] flex flex-col gap-[var(--spacing-3xs)]" aria-label="Main navigation">
          {NAV_ITEMS.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
